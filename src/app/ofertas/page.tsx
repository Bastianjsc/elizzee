...
export default function OfertasPage() {
  ...
  const [products, setProducts] = useState<ProductoBase[]>([]);
  ...
  // 1. PRIMERO: Filtramos de toda la base de datos SOLO los que tienen descuento
  let filteredProducts = products.filter(
    (product) => product.discountPrice !== undefined && product.discountPrice !== null
  );

  // 2. SEGUNDO: Aplicamos los filtros de precio del usuario
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] w-full flex items-center justify-center text-black">
        <p className="text-red-700 tracking-widest uppercase text-sm font-light animate-pulse">
          Buscando ofertas exclusivas...
        </p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {filteredProducts.map((product) => (
              <ProductCard
                ...
                // La magia se mantiene: pasamos variables que pueden existir o no
