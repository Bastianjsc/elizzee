...
export default function BrilloPage() {
  ...
  const [products, setProducts] = useState<BaseBrillo[]>([]);
  ...
  let filteredProducts = products.filter((product: BaseBrillo) => {
    // PRIMERO: Asegurarnos de que solo muestre la categoría "base-brillo"
    if (product.category !== "base-brillo") return false;
  ...
  filteredProducts = [...filteredProducts].sort((a, b) => {
    ...
    if (sortBy === "tamano-mayor") {
      // Protegemos con un "0" por si algún tamaño viene vacío de la BD
    ...
    return (
      <div className="min-h-screen bg-[#FAFAFA] w-full flex items-center justify-center text-black">
        <p className="text-gray-500 tracking-widest uppercase text-sm font-light animate-pulse">
          Cargando bases y brillos profesionales...
        </p>
      </div>
    );
  }
