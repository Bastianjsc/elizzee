// src/app/lamparas/page.tsx
"use client";

import React, { useState, useEffect } from "react";
import ProductCard from "@/components/ProductCard";
import FiltrosProductos from "@/components/FiltrosProductos";
import { Lampara } from "@/data/tipos";

export default function LamparasPage() {
  // --- NUEVOS ESTADOS PARA MONGODB ---
  const [products, setProducts] = useState<Lampara[]>([]);
  ...
  let filteredProducts = products.filter((product: Lampara) => {
    // PRIMERO: Asegurarnos de que solo muestre la categoría "lamparas"
    if (product.category !== "lamparas") return false;
  ...
  filteredProducts = [...filteredProducts].sort((a, b) => {
    ...
    if (sortBy === "tamano-mayor") {
      // Le enseñamos al código cuánto "pesa" cada palabra para poder ordenarlas
      const sizeWeight = (size: string | undefined) => {
        if (size === "Profesional") return 3;
        if (size === "Mediana") return 2;
      ...
      <div className="pt-12 pb-24 px-6 md:px-12 max-w-[1440px] mx-auto">
        ...
        <div className="flex flex-col items-center mb-12 text-center">
          ...
          <p className="text-gray-500 text-sm max-w-2xl font-serif italic">
            Tecnología de secado rápido y profesional. Asegura un curado perfecto,
            uniforme y duradero para tu manicura.
          </p>
